...
class Pix2PixHDModel(BaseModel):
    ...
    def ger_average_color(self, mask, arms):
        color = torch.zeros(arms.shape).cpu()
    ...
    def encode_input(self, label_map, clothes_mask, all_clothes_label):
        ...
        masked_label = masked_label.cpu().scatter_(
            1, (label_map * (1 - clothes_mask)).data.long().cpu(), 1.0)

        c_label = torch.FloatTensor(torch.Size(oneHot_size)).zero_()
        c_label = c_label.cpu().scatter_(
            1, all_clothes_label.data.long().cpu(), 1.0)
    ...
    def encode_input_test(self, label_map, label_map_ref, real_image_ref, infer=False):
        ...
        if self.opt.label_nc == 0:
            ...
            input_label_ref = label_map_ref.data.cpu()
        else:
            ...
            input_label_ref = torch.FloatTensor(
                torch.Size(oneHot_size)).zero_()
            input_label_ref = input_label_ref.cpu().scatter_(
                1, label_map_ref.data.long().cpu(), 1.0)
        ...
        real_image_ref = Variable(real_image_ref.data.cpu())
    ...
    def forward(self, label, pre_clothes_mask, img_fore, clothes_mask, clothes, all_clothes_label, real_image, pose, grid, mask_fore):
        ...
        arm1_mask = torch.FloatTensor(
            (label.cpu().numpy() == 11).astype(np.float)).cpu()
        arm2_mask = torch.FloatTensor(
            (label.cpu().numpy() == 13).astype(np.float)).cpu()
        pre_clothes_mask = torch.FloatTensor(
            (pre_clothes_mask.detach().cpu().numpy() > 0.5).astype(np.float)).cpu()
        ...
        fake_cl_dis = torch.FloatTensor(
            (fake_cl.detach().cpu().numpy() > 0.5).astype(np.float)).cpu()
        fake_cl_dis = morpho(fake_cl_dis, 1, True)

        new_arm1_mask = torch.FloatTensor(
            (armlabel_map.cpu().numpy() == 11).astype(np.float)).cpu()
        new_arm2_mask = torch.FloatTensor(
            (armlabel_map.cpu().numpy() == 13).astype(np.float)).cpu()
